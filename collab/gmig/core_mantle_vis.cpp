...
int main(int argc, char* argv[])
{
    ...
    bbox_t bounds = core_mantle::read_text(_vertices, filename, false);
    std::cout << _vertices.size() << " vertices imported across region "
              << bounds << '\n';
    ...
    for (int i=0 ; i<_vertices.size() ; ++i) {
        vertices.insert(vertices[i]);
    }
    
    // data indexing
    indexed_by_longitude& longitudes = vertices.get<longitude>();
    indexed_by_section_index& sections = vertices.get<section_index>();
    ...
    index_t last_section_index = longitudes.begin()->section_index;
    scalar_t last_longitude = longitudes.begin()->longitude();
    typedef indexed_by_longitude::iterator long_iter_t;
    for (long_iter_t it=longitudes.begin() ; it!=longitudes.end(); ++it)
    {
        const vertex_t& v = *it;
        index_t cur_section_index = v.section_index();
        scalar_t cur_longitude = v.longitude();
        
        if (cur_section_index != last_section_index ||
            cur_longitude != last_longitude) {
