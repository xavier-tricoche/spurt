...
struct sparse_matrix {
	...
	T operator()(int i, int j) const {
		...
		if (row_starts == row_ends) return T(0);
		...
		if (j < first_col || j > last_col) return T(0);
		else if (j == first_col) return __cols[row_starts].second;
		else if (j == last_col) return __cols[row_ends-1].second;
