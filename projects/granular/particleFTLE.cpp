...
int main(int argc, const char* argv[])
{
    ...
    size_t nbp = nin_start->axis[1].size;
    ...
    // allocate volume
    float* data = (float*)calloc(2 * nbp, sizeof(float));
    ...
    for (int n = 0 ; n < nbp ; ++n) {
        ...
        nvis::vec3 x = to_vector<3>(&coord_start[3*n]);
        ...
        data[2*n  ] = sane(ftle(coef_fwd));
        data[2*n+1] = sane(ftle(coef_bwd));
    }
    
    std::cerr << "Number of ftle values computed: " << 2*nbp << '\n';
