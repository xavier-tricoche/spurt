                   ...
                   const size_t max_nb_triangles)
{
    ...
    typedef std::set<triangle_info, Gt_triangle_priority>       priority_list_type;
    ...
    try {
        ...
        for (size_t i = 0 ; i < nb_triangles; ++i) {
            ...
            if (p > 0) {
                ...
                priority_list.insert(triangle_info(i, p));
            }
        }
        ...
        while (nb_triangles < max_nb_triangles && !priority_list.empty()) {
            ...
            priority_list_type::iterator selected = priority_list.begin();
            const triangle_info& info = *selected;
            if (info.version < current_version[info.index]) {
                // we have already replaced that entry by a more recent one. remove it
                priority_list.erase(selected);
                continue;
                ...
                // std::cerr << "seeding new point at " << seed << " in triangle #" << info.index << " = (" << triangle[0] << ", "
                ...
                for (int n = 0 ; n < modified.size() ; ++n) {
                    ...
                    priority_list.insert(triangle_info(id, p, current_version[id]));
                    // std::cerr << "inserted a new triangle for refinement\n";
                }
            }
        ...
        priority_list_type::iterator iter;
        for (iter = priority_list.begin() ; iter != priority_list.end() ; ++iter) {
            const triangle_info& info = *iter;
            if (info.version < current_version[info.index]) {
                // we have already replaced that entry by a more recent one. remove it
                priority_list.erase(iter);
            } else if (info.priority <= 0) {
                // invalid priority value. remove it
                priority_list.erase(iter);
            }
    ...
    if (!priority_list.empty()) {
        ...
        std::cerr << "worst offender has priority value: " << priority_list.begin()->priority << "\n";
    }
