...
struct point_locator {
    ...
    typedef kdtree_type::const_iterator             const_iterator;
    
    point_locator() {}
    
    point_locator(const point_locator& pl) : kdtree(pl.kdtree) {}
    
    int find_close_point(const nvis::vec2& x, int id, bool insert = true) const {
        std::pair<const_iterator, double> nearest =
            kdtree.find_nearest(data_point_type(x, id));
        if (insert) {
            kdtree.insert(data_point_type(x, id));
        }
        if (kdtree.empty()) {
            return 0;
        } else {
...
set_bounding_box(const std::pair<point_type, data_type> boundary[4])
{
    ...
    for (int i = 0 ; i < 4 ; ++i) {
        ...
        __locator.find_close_point(__verts[i], i);
    }
...
inline void triangulation<T1, T2>::triangulate_new_points(index_type first_id)
{
    ...
    for (index_type n = 0 ; n < nb_pts ; ++n) {
        ...
        index_type close_id = __locator.find_close_point(x, p_id);
        index_type tri = 0;
        if (__vert_to_tris[close_id].empty()) {
...
size_t avg_crossed_tris = 0;
size_t nb_success_search = 0;
        ...
        std::set<size_t>& visited) const
{
    ...
    if (been_there.size() == 200) {
        std::cerr << "we have currently crossed 200 triangles and average is "
                  << (double)avg_crossed_tris / (double)nb_success_search << std::endl;
    }
    ...
    while (true) {
        ...
        if (inside) { // all edges have tested positive
            other = -1;
            avg_crossed_tris += been_there.size();
            ++nb_success_search;
