...
typedef xavier::triangulation<point_data, xavier::point_locator>        mesh_type;
...
extern std::vector< std::vector< xavier::fixpoint > > all_chains;
extern xavier::map_metric metric;

extern std::vector<std::pair<unsigned int, nvis::vec2> > probed_seeds;

extern unsigned int verbose_level;

double min_norm(unsigned int& pid, unsigned int oid, const xavier::map_metric& metric, unsigned int p);
...
min_norm(unsigned int& pid, unsigned int oid, const xavier::map_metric& metric, unsigned int p)
{
    if (oid>=xavier::__map_orbits.size()) {
        throw std::runtime_error("min_norm: requested min norm on invalid orbit");
    }
    const xavier::orbit& orbit = xavier::__map_orbits[oid];
    if (p >= orbit.size()) {
    ...
    for (int id = 0 ; id < orbit.size() ; ++id) {
        double norm = nvis::norm(xavier::vector_value(oid, id, p, metric));
        if (norm<min) {
            min=norm;
            pid = id;
        }
