...
class fixed_sorted_vector : public fixed_vector<T, N> {
public:
    typedef fixed_vector<T, N>     base_type;
    typedef fixed_sorted_vector<T, N>    self_type;
    typedef Comp                        less_than;
    
    fixed_sorted_vector() : base_type() {}
    
    fixed_sorted_vector(const base_type& v) : base_type(v) {
        std::sort(this->begin(), this->end(), less_than());
    }

    fixed_sorted_vector(const T& v) : base_type(v) {}
    ...
    template<typename V>
    fixed_sorted_vector(const V& v) : base_type(&v[0], &v[N]) {
        std::sort(this->begin(), this->end(), less_than());
    }
    
    bool operator==(const self_type& sa) const {
        return all(static_cast<base_type>(*this) == 
                         static_cast<base_type>(sa));
    }
    
    bool operator<(const self_type& sa) const {
        lexicographical_order _order;
