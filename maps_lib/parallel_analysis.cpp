...
void initialize(int argc, char* argv[])
{
    ...
    hestOptAdd(&hopt, "edge",   "show edges",           airTypeInt,     0,  0,  &show_edges,NULL,           "show problematic edges");
...
void compute_cell_index(const std::vector<p_cell_type>& cells)
{
    ...
    std::cerr << "computation of cell indices took " << _timer.elapsed() << " s.                          \n";
                    ...
                    bool prefound = false)
{
    ...
    #pragma omp parallel
    {
        ...
        for (int n=0 ; n<cells.size() ; ++n) {
            ...
            map_type* amap = pmap.clone();
            amap->precision(1.0e-10);
            
            fixpoint fp;
            nvis::bbox2 bounds;
            bounds.min() = (*_plane)(cell_id);
            bounds.max() = (*_plane)(cell_id + nvis::ivec2(1,1));
            bool found = meta_newton(*amap, _plane_metric, bounds, x, 5, period,
                                     fp, chain, 1.0e-3, 0, false, 50, prefound);
...
#pragma openmp atomic
                ...
                std::ostringstream os;
                os << "fixed point on " << period << "-chain found" << std::endl;
                std::cerr << os.str();
            }
    ...
    #pragma omp parallel
    {
        ...
        for (int n=0 ; n<_boxes_indices.size() ; ++n) {
            ...
            bool found = meta_newton(*amap, _plane_metric, bounds, x, 6, period,
                                     fp, chain, 1.0e-3, 0, false, 50, false);
...
bool load_fixpoints(const std::string& name)
{
    ...
    std::cerr << "verifying the linear type of each fixpoint\n";
    map_type pmap(*_field);
    pmap.precision(1e-10);
#pragma openmp
    {
        ...
        for (int i=0 ; i<N ; ++i) {
            fixpoint tmp;
            map_type* amap = pmap.clone();
            bool consensus = linear_analysis(*amap, _fixpoints[i].K, _plane_metric, _fixpoints[i].pos, tmp,
                                             0.05, 0.005);
            if (!similar(tmp, _fixpoints[i])) {
                std::ostringstream os;
                os << "fixpoint at " << _fixpoints[i].pos << " has been changed from "
                   << (_fixpoints[i].saddle ? "saddle" : "center") << " to "
                   << (tmp.saddle ? "saddle" : "center") << std::endl;
                std::cerr << os.str();
            }
            _fixpoints[i] = tmp;
        }
...
void draw(void)
{
    ...
    for (int i=0 ; i<_fixpoints.size() ; ++i) {
        fixpoint& fp = _fixpoints[i];
        if (fp.saddle) {
            nvis::vec2 x = fp.pos + 0.25*fp.evec[0];
            glVertex2f(x[0], x[1]);
            x -= 0.5*fp.evec[0];
            glVertex2f(x[0], x[1]);
            x = fp.pos + 0.25*fp.evec[1];
            glVertex2f(x[0], x[1]);
            x -= 0.5*fp.evec[1];
            glVertex2f(x[0], x[1]);
        }
...
int main(int argc, char** argv)
{
    ...
    // if (!done && strcmp(topo, "none")) {
    //     std::cerr << "about to save topological information to file\n";
    //     save_chains(topo);
    // }
