...
double lmax(size_t n, const raster_t<vec6>& pos, const raster_t<bool>& _valid, const cr3bp& field)
{
    ...
    for (unsigned int dim = 0 ; dim < 2 ; ++dim) {
        ...
        dxi[dim] = 1. / (dh[dim] * (double)(max - min)) *
            ( field.project(pos[indices[dim][max]]) -
              field.project(pos[indices[dim][min]]));
    ...
    double lmaj = 0.5*(a + c + sqrt((a - c)*(a - c) + 4*b*b));
...
void output(const std::string& name, const raster_t<T>& data) {
    ...
    size_t memsz=xavier::data_traits<T>::mem_size();
    if (sz>1) os << sz << " ";
            ...
            progress_counter = 0;
            {
                ...
                for (size_t n=r.begin(); n!=r.end(); ++n) {
                    ...
                    if (tangent[n]<2) {
                        ...
                        if (std::isnan(ftle_fwd)) ftle_fwd = 0;
                        if (std::isnan(ftle_bwd)) ftle_bwd = 0;
