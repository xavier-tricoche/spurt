...
struct ODE_state {
    ODE_state() : m_stopped(false) {}
    ODE_state(const vec2& p0, value_t t0=0) 
        : m_time(t0), m_pos(p0), m_stopped(false) {}
...
{       
    ...
    observer(ODE_state& state, std::vector<vec2>& pos, value_t dt=60) 
        : m_state(state), m_pos(pos) {}
    
    void initialize(const vec2& p0, value_t t0) {
        m_state = ODE_state(p0, t0);
        m_pos.clear();
        m_pos.push_back(p0);
    }
