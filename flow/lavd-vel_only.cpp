...
bbox_t region; // ROI, invalid bounds by default
bbox_t domain; // entire domain
...
{   
    ...
    NrrdVectorField& m_vel; // vorticity computed on the fly from velocity
    
    value_t vorticity_deviation(const vec2& x, value_t t) {
        try {
            value_t w = m_vel.vorticity( vec3(x[0], x[1], t ) );
    ...
    observer(LAVD_state& state, std::vector<vec2>& pos, 
             NrrdVectorField& velocity, 
             NrrdScalarField<1>& average_vorticity) 
        : m_state(state), m_pos(pos), m_vel(velocity), 
...
{                    
    ...
    _log_(1)
        ...
        << "x spacing: " << current_velocity_volume->axis[1].spacing << '\n'
        << "y spacing: " << current_velocity_volume->axis[2].spacing << std::endl;
    ...
    size_t nx = current_velocity_volume->axis[1].size;
    size_t ny = current_velocity_volume->axis[2].size;
    ...
    std::vector< std::shared_ptr< NrrdVectorField  > > vfs(nb_threads);
    ...
    for (size_t n=0; n<n_used; ++n, base_n+=nsamples) {
        value_t sum=0;
        value_t t = current_t_min + n*t_between_files;
        
        double* vort_array = (double*)calloc(nsamples, sizeof(double));
        double* vel_array = (double*)calloc(3*nsamples, sizeof(double));
        double* jac_array = (double*)calloc(9*nsamples, sizeof(double));
        ...
        #pragma omp parallel
        {
            ...
            for (size_t k=0; k<nsamples; ++k) {
                ...
                if (!thread) progress.update(n*nsamples+k);
                int j=k/ncols;
                int i=k%ncols;
                
                value_t x=minx + min_offset_x + i*dx;
                value_t y=miny + min_offset_y + j*dy;
                value_t vort = vfs[thread]->vorticity(vec3(x,y,t));
                vort_array[k] = vort;
                
                vec3 vel;
                (*vfs[thread])(vec3(x,y,t), vel);
                vel_array[3*k+0] = vel[0];
                ...
                jac_array[9*k+2] = J(0,2);
                jac_array[9*k+3] = J(1,0);
                jac_array[9*k+4] = J(1,1);
                jac_array[9*k+5] = J(1,2);
                jac_array[9*k+6] = J(2,0);
                jac_array[9*k+7] = J(2,1);
                jac_array[9*k+8] = J(2,2);
                
                #pragma omp atomic
                sum += vort; // vorticity
            }
        }
        av_array[n] = sum/static_cast<value_t>(nsamples);
        _log_(1) << std::setprecision(12) << "average vorticity[" << n << " (t=" << t << ")]=" << av_array[n] << std::endl;
        
        // debug: export vorticity field
        {
            ...
            double _spc[2] = { dx, dy };
            double _min[2] = { minx + min_offset_x, miny + min_offset_y };
            int _center[2] = { nrrdCenterNode, nrrdCenterNode };
            nrrdAxisInfoSet_nva(nout, nrrdAxisInfoSpacing, _spc);
            nrrdAxisInfoSet_nva(nout, nrrdAxisInfoMin, _min);
        ...
        // debug: export velocity field
        {
            ...
            // Per-axis info
            int center = nrrdCenterNode;
            double _spc[3] = { AIR_NAN, dx, dy };
            double _min[3] = { AIR_NAN, minx + min_offset_x, miny + min_offset_y };
            int _center[3] = { nrrdCenterUnknown, nrrdCenterNode, nrrdCenterNode };
            nrrdAxisInfoSet_nva(nout, nrrdAxisInfoSpacing, _spc);
            nrrdAxisInfoSet_nva(nout, nrrdAxisInfoMin, _min);
            nrrdAxisInfoSet_nva(nout, nrrdAxisInfoCenter, _center);
...
void export_results(double current_time, double wall_time, double cpu_time, bool final) {    
    ...
    if (long_name) {
        os << name_out << "_started_at_" << start_time_str << "_flowmap+lavd_" << std::setw(4) << std::setfill('0') << current_time/HOUR << "h_";
        os << res[0] << "x" << res[1] << "_" << std::setprecision(2)
           << std::scientific << eps << ".nrrd";
        os.flags(default_settings);
    }
    else {
        os << name_out << "_fmap+lavd_" << current_time/HOUR << "h.nrrd";
    }
    ...
    os << "t=" << current_time << "; ";
    os << "epsilon=" << eps << "; ";
    os << "ODE solver: Dormand-Prince; ";
    os << "sampling=" << res[0] << "x" << res[1] << "; ";
    os << "bounds=" << region.min() << "->" << region.max() << "; ";
    os << "vorticity sampling period=" << dt << "; ";
...
int main(int argc, const char* argv[])
{
    ...
    while (info_file.good()) {
        ...
        if (info_file.fail()) break;
        std::istringstream iss(line);
        std::string velname;
        iss >> velname; // discared the rest if any
    ...
    _log_(1) << "domain of definition=\n" << domain << std::endl;
    _log_(1) << "region of interest=\n" << region << std::endl;
    ...
    import_data(velocity_filenames, t_init/HOUR/3);
    ...
    for (int i=0; i<nb_threads; ++i) {
        vf_copies[i] = shared_ptr< NrrdVectorField >(new NrrdVectorField(current_velocity_volume, std::string("velocity volume for thread #") + std::to_string(i), true));
    ...
    while ( true ) {
        ...
        #pragma omp parallel
        {
            ...
            for (size_t n = 0 ; n < nb_samples ; ++n) {
                ...
                NrrdScalarField<1> my_avg_vorticityf(current_average_vorticity_vector);
                observer an_observer(all_states[n], all_trajectories[n], 
                                     *vf_copies[thread], my_avg_vorticityf);
                
                if (initial_loop) {
                    _log_(1) << "initializing observer at " << x0 << std::endl;
                    try {
                ...
                try { 
                    ...
                    _log_(1) << "unknown exception thrown!" << std::endl;
                }
                _log_(1) << rhs.m_counter << " RHS evaluations for this IC"
                    << std::endl;
            }
        ...
        if (current_target_time == t_max) break;
            ...
            import_data(velocity_filenames,
                        last_time_step-2*support_radius);
            for (int i=0; i<nb_threads; ++i) {
                vf_copies[i] = shared_ptr< NrrdVectorField >(new NrrdVectorField(current_velocity_volume, std::string("velocity volume for thread #") + std::to_string(i)));
                rhs_copies[i] = shared_ptr< NrrdODERHS >(new NrrdODERHS(*vf_copies[i], sample_counter[i], domain, max_rhs_evals));            
            }
        }
