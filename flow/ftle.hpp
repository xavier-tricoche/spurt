...
struct file_name {
    ...
    const char* make(const std::string& what, int id, float length = 0) {
        os.clear();
        os.str("");
        os << basename << "-" << what << "--" << id;
        if (length > 0) {
...
{
    spurt::fixed_vector<spurt::vec3, 3> J(flowmap.derivative(flowmap.grid()(n)));
    spurt::mat3 M;
    for (int i = 0 ; i < 3 ; ++i) {
        for (int j = 0 ; j < 3 ; ++j) {
            M(i, j) = J[i][j];
...
{
    spurt::ivec3 coord = flowmaps[0].grid()(n);
    spurt::mat3 M[2];
    spurt::vec3 dmap[2];
