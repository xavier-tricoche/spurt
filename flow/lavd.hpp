...
typedef spurt::fixed_vector< value_t, 1 > vec1;
typedef spurt::fixed_vector< value_t, 2 > vec2;
typedef spurt::fixed_vector< value_t, 3 > vec3;
typedef spurt::fixed_vector< value_t, 4 > vec4;
typedef spurt::fixed_vector< size_t, 1 > lvec1;
typedef spurt::fixed_vector< size_t, 2 > lvec2;
typedef spurt::fixed_vector< size_t, 3 > lvec3;
typedef nvis::bounding_box< vec2 >     bbox_t;

typedef spurt::fixed_vector< long, 2 > coord_t;
...
inline T nrrd_value(const Nrrd* nin, size_t n) {
    return spurt::nrrd_data_wrapper<value_t>(nin)[n];
}

template<>
inline vec3 nrrd_value< vec3 >(const Nrrd* nin, size_t n) {
    vec3 v(0);
    v[0] = spurt::nrrd_data_wrapper<value_t>(nin)[3*n];
    v[1] = spurt::nrrd_data_wrapper<value_t>(nin)[3*n+1];
    return v;
}
...
inline void get_spatial_info(bbox_t& bounds, spurt::vec2& spc, const std::string& filename, int offset=0) {
...
void print(const Nrrd* nin) {
    ...
    _log_(2) << "dim=" << nin->dim << '\n';
    _log_(2) << "type=" << nin->type << " (" << spurt::nrrd_type_name(nin->type)  << ")" << '\n';
...
struct NrrdScalarField {
    ...
    typedef spurt::fixed_vector< value_t, N > pos_t;
    typedef const void* address_t;
    ...
    static Nrrd* make3d(Nrrd* nin) {
        ...
        Nrrd* nout=nrrdNew();
        if (nrrdWrap_nva(nout, data, nrrd_value_traits_from_type<value_t>::index, 3, sz)) {
            throw std::runtime_error(spurt::error_msg("Unable to create Nrrd"));
        }
...
upsample(Nrrd* nin, const spurt::fixed_vector<size_t, N>& up,
...
upsample_scalar(Nrrd* nin, const spurt::fixed_vector<size_t, N>& up,
                const std::string& what="") 
{
...
upsample_vector(Nrrd* nin, const spurt::fixed_vector<size_t, 3>& up,
                const std::string& what="") 
{
...
size_t compute_support_radius() {
    ...
    Nrrd* nin=nrrdNew();
    if (nrrdWrap_nva(nin, data, nrrd_value_traits_from_type<value_t>::index, 
                     1, sz)) {
        throw std::runtime_error(spurt::error_msg("unable to compute support radius"));
    }
...
Nrrd* create_nrrd_volume(std::vector< Nrrd* >& slices, value_t min, value_t spc) {
    ...
    if (nrrdWrap_nva(nout, data, nrrd_value_traits_from_type<value_t>::index, dim, sizes)) {
                    ...
                    const std::vector< LAVD_state >& all_states) 
{
    ...
    spurt::writeNrrdFromContainers(lavd, filename, 
                      ...
                      const std::string& velocity_name)
{
    if (!border_mask_name.empty()) {
        return spurt::readNrrd(border_mask_name);
    }
    
    Nrrd* vel = spurt::readNrrd(velocity_name);
                 ...
                 bool is_scalar=true)
{
    ...
    size_t sz[2] = {M, N};
    spurt::writeNrrd(mask_array, "region_mask.nrrd", nrrdTypeDouble, 2, sz);
