...
class index_shifter
{
    typedef matrix<Array> __wrapper_type;
public:
    typedef data_traits<Array> traits_type;
    static const size_t dimension = traits_type::size();
    typedef typename traits_type::value_type size_type;
    typedef Array array_type;
    typedef Array coord_type;
...
private:
    ...
    friend std::ostream &operator<<(std::ostream &os, const index_shifter_new &is)
    {
        os << "[ [sizes: " << is.m_sizes
           << "]\n[dim shifts: "
           << is.m_dim_shifts
...
public:
    ...
    template< typename Resolution_, typename Bounds_ >
    raster_grid(const Resolution_& resolution, const Bounds_& bounds,
                bool cell_based=false);
    template< typename Resolution_, typename Point_, typename Vector_>
    raster_grid(const Resolution_& resolution, const Point_& origin,
                const Vector_& spacing, bool cell_based=false);
    // vertex access
