...
inline typename index_shifter_new<Array>::coord_type
index_shifter_new<Array>::local_coords(size_t local_id) const
{
    coord_type r;
    std::bitset<sizeof(size_type)> bits(local_id);
    for (size_t i = 0; i < r.size(); ++i)
...
template <typename Array>
index_shifter_new<Array>::index_shifter_new(const array_type &sizes)
    : m_sizes(sizes)
{
    std::cout << "shifter m_sizes=" << m_sizes << '\n';
    size_type shift = 1;
...
template <typename Array>
index_shifter_new<Array>::index_shifter_new(const index_shifter_new<Array> &other)
    : m_sizes(other.m_sizes), m_dim_shifts(other.m_dim_shifts),
      m_cell_shifts(other.m_cell_shifts) {}

template <typename Array>
typename index_shifter_new<Array>::size_type
index_shifter_new<Array>::
operator()(size_type base, size_type i) const
{
    return base + i;
}
...
template <typename Array>
typename index_shifter_new<Array>::size_type
index_shifter_new<Array>::lift(size_type base, size_type d) const
{
    return base + m_dim_shifts[d];
}
...
template <typename Array>
typename index_shifter_new<Array>::size_type
index_shifter_new<Array>::index(size_type base, size_type local_id) const
{
    return base + m_cell_shifts[local_id];
}
...
raster_grid<Dim_, Scalar_, Size_>::operator()(const coord_type& ids) const
{
    ...
    vec_type _ids = ids.template cast<scalar_type>();
    return m_bounds.min() + (_ids.array()*m_spacing.array()).matrix();
}
...
bilinear(const vec_type& u, size_type a) const
{
    ...
    const vec_type& hi = u;
    vec_type one;
    vec_type lo = one.setConstant(1.) - hi;
    return
