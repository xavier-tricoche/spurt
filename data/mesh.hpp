...
namespace spurt { namespace mesh {
    
template<unsigned int N>
struct cell_type {
    typedef small_vector<size_t, N> ids_type;
...
const spurt::ivec4 voxel_faces[6] = {
    spurt::ivec4(0, 1, 2, 3), spurt::ivec4(4, 5, 6, 7),
    spurt::ivec4(0, 1, 5, 4), spurt::ivec4(1, 2, 6, 5),
    spurt::ivec4(2, 3, 7, 6), spurt::ivec4(3, 0, 4, 7)
};

const spurt::ivec3 voxel_vertices[8] = {
    spurt::ivec3(0, 0, 0), spurt::ivec3(1, 0, 0),
    spurt::ivec3(1, 1, 0), spurt::ivec3(0, 1, 0),
    spurt::ivec3(0, 0, 1), spurt::ivec3(1, 0, 1),
    spurt::ivec3(1, 1, 1), spurt::ivec3(0, 1, 1)
};

/// Legacy wrapper for raster_grid
template<typename Type, int N>
class uniform_mesh : public raster_grid<long, Type, N, spurt::small_vector<long, N>, spurt::small_vector<Type, N>>
{
...
public:
    typedef raster_grid<long, Type, N, spurt::small_vector<long, N>, spurt::small_vector<Type, N>> base_type;
    typedef uniform_mesh<Type, N>            self_type;
    ...
    uniform_mesh(const base_type& other) : base_type(other), 
        _periodic(false), _verbose(false) {
        _metric.bounds() = other.bounds();
        _metric.periodic() = _periodic;
    }

    uniform_mesh(const ivec_type& size, const vec_type& spacing)
    ...
    uniform_mesh(const ivec_type& size, const bounds_type& _bounds)
        : base_type(size, _bounds), _periodic(false), _verbose(false) {
        _metric.bounds() = this->bounds();
        _metric.periodic() = _periodic;
    }
    
    uniform_mesh(const ivec_type& size, const vec_type& spacing, const bvec_type& periodic)
    ...
    uniform_mesh(const ivec_type& size, const vec_type& spacing, const vec_type& origin)
    ...
    ivec_type imodulo(const ivec_type& c) const {
        ...
        if (!spurt::any(_periodic)) return c;
        ivec_type pc(c);
        for (int i = 0 ; i < N ; ++i) {
            if (_periodic[i]) {
                ...
                throw std::runtime_error("mesh::imodulo(): invalid coordinates");
            }
        }
        return pc;
    }
    ...
    vec_type dmodulo(const vec_type& x) const {
        ...
        if (!this->bounds().inside(y))
            throw std::runtime_error("mesh::dmodulo(): invalid coordinates");
        return y;
    }
