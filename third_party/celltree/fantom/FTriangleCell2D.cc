...
//
// Project:   FAnToM
// Module:    $RCSfile: FTriangleCell2D.cc,v $
// Language:  C++
...
const FCell::geoInfo FTriangleCell2D::myGeoDescription =
  {
    // dimension
    2,
    // # vertices
...
FTriangleCell2D::FTriangleCell2D()
  : FCell2D(myGeoDescription,myIndices,myPositions,myPositionData,myTensors)
{}

//--------------------------------------------------------------------------- 

FTriangleCell2D::FTriangleCell2D( const vector<FIndex>& vertIds )
  : FCell2D(myGeoDescription,myIndices,myPositions,myPositionData,myTensors),lastPos(2)
{
  for (char i=0; i<3; i++)
...
FTriangleCell2D::FTriangleCell2D( const FIndex*vertIds )
  : FCell2D(myGeoDescription,myIndices,myPositions,myPositionData,myTensors),lastPos(2)
{
  for (char i=0; i<3; i++)
    vertexIndices[i]=vertIds[i] ;
...
FCell*  FTriangleCell2D::getClone() const
{
  try 
    {
      return new FTriangleCell2D( vertexIndices );
    }
  catch (FException& e) 
    {
      e.addTraceMessage("FCell*  FTriangleCell2D::getClone() const");
      throw;
      return (FCell*) 0;
    }
}
...
void FTriangleCell2D::interpolate(FTensor& result, 
				  const FPosition& position) const
{
  try {
    ...
    e.addTraceMessage("void FTriangleCell2D::interpolate(FTensor& result, const FPosition& position)");
    throw;
  }
} 
...
void FTriangleCell2D::derivatives(FTensor& result, const FPosition&) const
{
  try {
    if (!positions || !tensors) {
      FException e("ERROR: missing position and/or tensor information");
    ...
    e.addTraceMessage("void FTriangleCell2D::derivatives(FTensor& result, const FPosition& position)");
    throw;
  }
}
...
bool FTriangleCell2D::isInside(const FPosition& position) const
{
  try {
    if (!positions) {
...
// 	 << ", " << b[2] << endl;
    ...
    e.addTraceMessage("bool FTriangleCell2D::isInside(const FPosition& position) const");
    throw;
  }
}
...
bool  FTriangleCell2D::isInside(const FPosition& pos,
...
// 	   << endl << "FALSE" << endl;
    ...
    {
      e.addTraceMessage("bool FTriangleCell2D::isInside(const FPosition& pos, const vector< FPosition >& vertices)");
...
void FTriangleCell2D::buildBoundingBox(void) const
{
  try {
    ...
    minY = positions[0][1];
    maxY = minY;
    for (positive i=1 ; i<3 ; i++) {

      if (positions[i][0] < minX) 
	minX = positions[i][0];
      else if (positions[i][0] > maxX) 
	maxX = positions[i][0];
    ...
    e.addTraceMessage("void FTriangleCell2D::buildBoundingBox(void)");
    throw;
  }
}
...
void FTriangleCell2D::barycentricCoordinates(double b[3], 
					     const FPosition& position) const
{
  try {
...
//       cout << "WARNING WARNING WARNING" << endl;
    ...
    e.addTraceMessage("void FTriangleCell2D::barycentricCoordinates(double b[3], const FPosition& position) const");
    throw;
  }
}
...
void FTriangleCell2D::derivBaryCoord(double dbdx[3], double dbdy[3]) const
{
  try {
    ...
    e.addTraceMessage("void FTriangleCell2D::derivBaryCoord(double dbdx[3], double dbdy[3]) const");
    throw;
  }
}
...
FCell::CellType FTriangleCell2D::getCellType(void) const
{
  return FCell::TRIANGLE_2D;
}

//--------------------------------------------------------------------------- 
 
void FTriangleCell2D::getZeros(list<FAMSingularPoint>& result) const
{
  try {
    ...
    if (denom != 0.) {
      ...
      cerr << "singular system encountered in void FTriangleCell2D::getZeros()"
	   << endl;
      return;
    }
    ...
    e.addTraceMessage("void FTriangleCell2D::getZeros(list<FAMSingularPoint>& result) const");
    throw;
  }
}
...
positive FTriangleCell2D::memSize() const
{
  return
    (
     +3//lastPos
