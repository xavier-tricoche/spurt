      ...
      if(l>1)
	{
	  ...
	  if(scale == 0.0)
	      ...
	      for(j=1; j<=l; j++)
		{
		  ...
		  e[j-1] = g = e[j-1] - hh*f;
		  for(k=1; k<=j; k++)
		    (*this)(j-1,k-1) -= (f*e[k-1] + g*(*this)(i-1,k-1));
		}
	    }
	}
      else
  ...
  for(i=1; i<=n; i++)
      ...
      if(d[i-1])
	{
	  for(j=1;j <= l; j++)
	    {
	      g = 0.0;
	      for(k=1; k <= l; k++) g += (*this)(i-1,k-1)*(*this)(k-1,j-1);
	      for(k=1; k <= l; k++) (*this)(k-1,j-1) -= g*(*this)(k-1,i-1);
	    }
	}
      d[i-1]=(*this)(i-1,i-1);
      (*this)(i-1,i-1) = 1.0;
      for(j=1; j <= l; j++) 
	(*this)(j-1,i-1) = (*this)(i-1,j-1) = 0.0;
    }
}
	  ...
	  if (m != l) 
	      ...
	      for (i=m-1;i>=l;i--)
		{
		  ...
		  for(k=1;k<=n;k++) 
		    {
		      f=(*this)(k-1,i);
		      (*this)(k-1,i) = s*(*this)(k-1,i-1) + c*f;
		      (*this)(k-1,i-1) = c*(*this)(k-1,i-1) - s*f;
		    }
		}
