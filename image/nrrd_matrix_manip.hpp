...
namespace spurt { namespace nrrd_manip { namespace detail {
    ...
    Nrrd* mat2nrrd(const std::vector< mat_t<T> >& mats,
        ...
        spurt::progress_display progress(show_progress);
        ...
        dims[3]=res[2];
        Nrrd *nout = nrrdNew();
        if (nrrdWrap_nva(nout, raster, nrrd_value_traits_from_type<T>::index, 
                         4, &dims[0])) {
    ...
    void do_FTLE(T* out_ptr, T1* in_ptr, size_t N, int Nrows, bool show_progress) {
        ...
        for (size_t i=0; i<N ; ++i, ++ftle, in_ptr+=ncoef) {
            new (&A) in_map_t(in_ptr, Nrows, Nrows);
    ...
    Nrrd* wrap_and_copy_header(const Nrrd* src, T* data, int valdim) {
        ...
        if (nrrdWrap_nva(nout, data, nrrd_value_traits_from_type<T>::index, dim, sizes)) {
            throw std::runtime_error(error_msg("wrap_and_copy_header"));
        }
