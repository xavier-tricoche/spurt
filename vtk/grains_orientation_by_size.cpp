...
void initialize(int argc, char* argv[])
{
    ...
    hestOptAdd(&hopt, "up",     "up/down",          airTypeBool,    0, 1, &param_updown,        "1",        "up/down color coding");
...
int main(int argc, char* argv[])
{
    ...
    std::string size_name(stat_base), id_name(stat_base);
    ...
    for (int i = 0 ; i < nb_grains - 2 ; ++i) {
        nvis::vec3 _d(orient[3*i], orient[3*i+1], orient[3*i+2]);
        // colors[i+1] = to_color(vec2col(_d));
        colors[i+1] = upcol(_d, 0.6, 6);
        std::cerr << "color is " << nvis::vec3(colors[i+1]) << '\n';
    }
    std::cerr << "colors set.\n";
    std::cerr << (param_updown ? "up/down" : "3D orientation") << " mode selected\n";
    ...
    for (double s = min ; s < max + 0.1*d ; s += d) {
        ...
        while (true) {
            ...
            if (!tmp3.size()) {
                ...
                std::cerr << "inserting color [" << ref_id << "] = " << nvis::vec3(colors[ref_id]) << '\n';
        ...
        os << info.movie_path << "grains_orientation_" << (param_updown ? "updown" : "on_sphere")
           << "_by_size_g=" << param_g << "_frame_" << std::setw(3) << std::setfill('0') << frame_counter++ << "_of_"
