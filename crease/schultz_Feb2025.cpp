...
struct h_root_finder {
    ...
    vector_type compute_h(const pos_type& p) {
        ...
        // std::cout << std::fixed << std::setprecision(12) << "compute_h(" << p << "): gv=" << gv << "hess=" << hess << '\n';
        return vector_h(gv, hess, m_theta);
    }

    std::pair<vector_type, vector_type> compute_h_dh(const pos_type& p, int dim) {
        // std::cout << std::fixed << std::setprecision(12) << "compute_h_dh(" << p << ") in dim " << dim << '\n';
        vector_type gv = m_gradient.value(p);
        matrix_type hess = m_hessian.value(p);
        tensor_type hess_prime = m_hessian.derivative(p);
        vector_type h = vector_h(gv, hess, m_theta);
        vector_type dh = vector_dh(gv, hess, hess.column(dim), hess_prime.layer(dim), m_theta);
                ...
                bool verbose=false)
    {
        ...
        for (int i=0; i<maxiter; ++i) {
            ...
            auto du = -spurt::inner(dh, h)/spurt::norm_square(dh);
            if (!lnsearch(u, h, du, p0, p1, verbose)) {
                break;
            }
            // if (verbose) {
            //     std::cout << std::fixed << std::setprecision(12) << "iteration " << i << ", u=" << u << ", |h|=" << spurt::norm(h) << '\n';
            // }
            if (spurt::norm(h) < eps) {
                if (verbose)
    ...
    if (inner(h0, h1) < 0) 
    {
        ...
        if (improve) {
            ...
            if (root.search(u, h, e)) {
                ...
                if (hfinal < 0.001*hinit) std::cout << "strong improvement\n";
                else if (hfinal < 0.01*hinit) std::cout << "significant improvement\n";
                else if (hfinal < 0.1*hinit) std::cout << "small improvement\n";
                else if (hfinal < hinit) std::cout << "marginal improvement\n";
                else std::cout << "no improvement\n";
            }
                           ...
                           const scalar_type& theta0) 
{
    if (crease_point_ids.size() % 2) {
    ...
    else if (crease_point_ids.size() == 4) {
#ifdef SPURT_DEBUG
        std::cout << "There are 4 crease points in input\n";
#endif
        ...
        // the remaining 2 points will be connected
        std::vector<size_type> others;
        for (int i=0; i<4 && others.size()<2; ++i) {
            auto id = crease_point_ids[i];
...
#ifdef SPURT_DEBUG
        std::cout << "ERROR: there are " << crease_point_ids.size() << " crease points in input" << '\n';
#endif
        return false;
    }
}
