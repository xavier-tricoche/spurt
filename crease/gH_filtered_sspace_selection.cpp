...
typedef scalar_raster::grid_type grid_type;

typedef image<size_type, scalar_type, 3, scalar_type, kernels::MitchellNetravaliBC> smooth_image;

scalar_type gauss(scalar_type x, scalar_type sigma) {
...
scalar_type default_lmin_treshold(const scalar_raster& values) {
    scalar_type mode = 
        spurt::mode(values.begin(), values.end(), 1024, 
                    std::numeric_limits<scalar_type>::min(), 0.);
...
scalar_raster ridge_strength(matrix_raster& hessian)
{
    size_type npts = hessian.grid().size();
    scalar_raster _lmin(hessian.grid());
    ProgressDisplay progress;
    progress.begin(npts, "Computing ridge strength from Hessian", 500);
    ...
    tbb::parallel_for(tbb::blocked_range<size_type>(0, npts),
        ...
        {
            const mat_type& H = hessian[n];
            vec_type evals;
            real_eigenvalues(evals, H);
            _lmin[n] = evals[2];
        }
    });
...
void ridge_strength(scalar_raster& lmin,
                    scalar_raster& input, 
                    scalar_type scale) 
{
    lmin.initialize(0);
    ...
    tbb::parallel_for(tbb::blocked_range<size_type>(0, npts),
        ...
        for (size_type n=r.begin(); n!=r.end(); ++n)
        {
            ...
            mat_type H = scsq * volume.second_derivative_in_voxel(ijk, u);
            vec_type lambdas;
            real_eigenvalues(lambdas, H);
                        ...
                        bool compute_strength=false)
{
    for ( auto info : blur_info ) {
        ...
        sample_at_fixed_scale(value, gradient, hessian, blurred, best_scale, s);
    }
    if (compute_strength) strength = ridge_strength(hessian);
}

void img_min(scalar_raster& inout_lmin, 
             scalar_raster& inout_scale,
             const scalar_raster& other_lmin,
             scalar_type other_scale) 
{
    size_type npts = inout_lmin.grid().size();
...
void select_scale(scalar_raster& lmin,
                  scalar_raster& best_scale,
                  const std::vector<std::pair<scalar_type, std::string>>& blur_info)
{
    scalar_raster other_lmin(lmin.grid());
    ...
    for ( auto info : blur_info ) {
        ...
        spurt::timer t;
        ridge_strength(other_lmin, blurred, s);
        img_min(lmin, best_scale, other_lmin, s);
...
scalar_raster filter_scale(scalar_raster& best_lmin, scalar_raster& best_scale,
                           scalar_type sigma_strength, scalar_type omega=0.5,
                           scalar_type sigma_scale=2)
{
    const grid_type& grid = best_lmin.grid();
    const coord_type& res = grid.resolution();
    size_type npts = grid.size();

    scalar_raster new_best_scale(best_scale.grid());

    ProgressDisplay progress;
    progress.begin(npts, "Filtering best scale", 500);
    tbb_progress_counter = 0;
    tbb::parallel_for(tbb::blocked_range<size_type>(0, npts),
                       [&](tbb::blocked_range<size_type> r) {
        for (size_type n=r.begin(); n!=r.end(); ++n)
        {        
            scalar_type total_w = 0;
            scalar_type scale = 0;

            coord_type ijk = grid.coordinates(n);

            scalar_type cur_scale = best_scale[n];
            scalar_type l3 = best_lmin[n];

            for (size_type i=-1; i<=1; ++i) {
                scalar_type x = ijk[0] + i;
                if (x < 0 || x > res[0]-1) continue;
                for (size_type j=-1; j<=1; ++j) {
                    scalar_type y = ijk[1] + j;
                    if (y < 0 || y > res[1]-1) continue;
                    for (size_type k=-1; k<=1; ++k) {
                        scalar_type z = ijk[2] + k;
                        if (z < 0 || z > res[2]-1) continue;
                        scalar_type s = best_scale(x,y,z);
                        scalar_type str = best_lmin(x,y,z);
                        scalar_type w = gauss(s-cur_scale, sigma_scale) * gauss(str-l3, sigma_strength);
                        scale += w * s;
                        total_w += w;
                    }
                }
            }
            new_best_scale[n] = (1-omega)*cur_scale + omega/total_w*scale;
        }
    });
    return new_best_scale;
}
...
int main(int argc, char* argv[]) 
{
    ...
    scalar_type cutoff;
    scalar_type omega = 0.5;
    scalar_type sigma_scale = 2;
    scalar_type sigma_strength=.2;
    bool verbose = false;
    ...
    try 
    {
        ...
        // parser.add_tuple<3>("scales", _scales, "Scales to consider (min, max, delta)", required_group);
        // parser.add_value("cutoff", cutoff, 4, "Cutoff (in multiple of stddev) of Gaussian kernel");
        parser.add_value("rate", omega, omega, "Scale smoothing rate");
        parser.add_value("scalew", sigma_scale, sigma_scale, "Standard deviation of scale gaussian smoothing weight");
        parser.add_value("strenthw", sigma_strength, sigma_strength, "Standard deviation of ridge strength gaussian smoothing weight (in \% of value range)", optional_group);
    ...
    select_scale(best_strength, best_scale, blurred_info);

    sample_scale_space(best_value, best_gradient, best_hessian, 
                       best_strength, best_scale, blurred_info);
    ...
    /*
    scalar_raster& best_lmin, scalar_raster& best_scale,
                           scalar_type omega=0.5, scalar_type sigma=2)
    */
    scalar_type min_str = default_lmin_treshold(best_strength);
    scalar_type max_str = *std::min_element(best_strength.begin(), best_strength.end());
    std::cout << "default lmin threshold = " << min_str << ", min lmin = " << max_str << '\n';
    sigma_strength *= (min_str-max_str);

    scalar_raster best_scale2 = filter_scale(best_strength, best_scale, sigma_strength, omega, sigma_scale);

    scalar_raster best_val2(first.grid());
    vector_raster best_grad2(first.grid());
    matrix_raster best_hess2(first.grid());
    scalar_raster best_str2(first.grid());
    sample_scale_space(best_val2, best_grad2, best_hess2, best_str2, best_scale2, blurred_info, true);
    std::cout << "Exporting data\n";

    save_as_nrrd(output_basename + "_scsp_filtered_value.nrrd", best_val2);
    save_as_nrrd(output_basename + "_scsp_filtered_strength.nrrd", best_str2);
    save_as_nrrd(output_basename + "_scsp_filtered_scale.nrrd", best_scale2);
    save_as_nrrd(output_basename + "_scsp_filtered_gradient.nrrd", best_grad2);
    save_as_nrrd(output_basename + "_scsp_filtered_hessian.nrrd", best_hess2, small_vector<scalar_type, 9>(0));
