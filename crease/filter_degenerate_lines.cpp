...
struct crease_point_type {
    crease_point_type() {}
    crease_point_type(const vec3& x) : p(x), val(0), str(0) {}
...
typedef crease_line_type::const_iterator        iterator_t;

void find_best_seed(vec3& x, double& mode, const crease_line_type& cl,
                    const spurt::MeasureWrapper& measure)
{
    mode = 0;
    for (iterator_t it = cl.begin() ; it != cl.end() ; ++it) {
        if (fabs(it->val) > mode) {
            mode = fabs(it->val);
            x = it->p;
        }
...
bool correct_point(vec3& x, const spurt::MeasureWrapper& measure)
{
...
double hausdorff(const crease_line_type& cl1, const crease_line_type& cl2)
{
    ...
    for (iterator_t it = cl1.begin() ; it != cl1.end() ; ++it) {
...
void draw(std::vector<vec3>& line, const vec3& seed,
          const spurt::MeasureWrapper& measure, int idx, double h)
{
    ...
    while (true) {
        vec3 x = fwd.back();
        vec3 t = fwd.back() - fwd[fwd.size()-2];
        // std::cout << "after " << fwd.size() << " fwd steps we are at " << x << " where mode is "
        // << measure.value(x) << std::endl;
        t /= norm(t);
        vec3 dx = measure.eigenvector(x, idx);
        if (inner(t, dx) < 0) {
            dx *= -1;
        }
        if (inner(t, dx) < min_cos) {
            std::cerr << "fwd tracking failed at " << x << " because dot product = " << inner(t, dx)
                      << std::endl;
            break;
        }
        x += h * dx;
    ...
    while (true) {
        vec3 x = bwd.back();
        vec3 t = bwd.back() - bwd[bwd.size()-2];
        ...
        if (inner(t, dx) < min_cos) {
            std::cerr << "bwd tracking failed at " << x << " because dot product = " << inner(t, dx)
                      << std::endl;
            break;
        }
        x += h * dx;
    ...
    line.clear();
    for (std::vector<vec3>::reverse_iterator it = bwd.rbegin() ; it != bwd.rend() ; ++it) {
        line.push_back(*it);
    }
...
// export results to file
    ...
    for (int i = 0 ; i < new_lines.size() ; ++i) {
        for (iterator_t it = new_lines[i].begin() ; it != new_lines[i].end() ; ++it) {
