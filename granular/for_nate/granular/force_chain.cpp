...
void initialize(int argc, char* argv[])
{
	...
	hestParseOrDie(hopt, argc - 1, argv + 1, hparm,
	               me, "Extract force chains in granular particle assembly and export principal stress tensors",
...
int main(int argc, char* argv[])
{
	...
	// export stress value
	{
		...
		spc[0] = spc[1] = airNaN();
		xavier::writeNrrd(stress_data, stress_f, nrrdTypeFloat, dims, spc);
	...
	for (int i = 0 ; i < seed_particles.size() ; ++i) {
		...
		if (marked.find(p.index) != marked.end()) continue;
